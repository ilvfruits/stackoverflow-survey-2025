---
layout: default
title: What Do Developers Really Say About Their Skills and Careers?
---
## What Do Developers Really Say About Their Skills and Careers?

### 📊 2025 Stack Overflow Survey Analysis

This site presents selected findings and reports based on analysis of the 2025 Stack Overflow Developer Survey.

### Featured Reports

- **[Can We Predict the Salary of a Developer?]({{site.baseurl}}/2025/12/22/salary-prediction-report/)**
  An end-to-end analysis and machine learning model training to explore how experience, role, location, and skills influence developer's salary.
  **[Image]({{site.baseurl}}/assets/images/alexander-grey-8lnbXtxFGZw-unsplash.jpg)**
  
- **[Which Programming Languages Command the Highest Salaries?]({{site.baseurl}}/2025/12/22/language-trends/)**
  A distribution-based analysis comparing salaries of developers using various programming languages.
  **[Image]({{site.baseurl}}/assets/images/chris-ried-ieic5Tq8YMk-unsplash.jpg)**
